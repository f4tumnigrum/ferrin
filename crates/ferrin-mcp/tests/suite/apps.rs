...
fn app_metadata_is_read_from_ui_and_legacy_keys() {
    ...
    assert_eq!(meta.resource_uri, "ui://w/a");
    ...
    assert_eq!(meta.resource_uri, "ui://w/b");
...
fn invalid_app_metadata_is_rejected() {
    ...
    assert!(matches!(
        app_tool_meta(&not_object),
        Err(McpError::InvalidArgument { .. })
    ));
    let missing_uri = tool("c", json!({"ui": {"visibility": ["model"]}}));
    assert!(matches!(
        app_tool_meta(&missing_uri),
        Err(McpError::InvalidArgument { .. })
    ));
}
...
fn tools_are_split_by_audience() {
    ...
    assert_eq!(names(&split.app_tools), vec!["both", "app_only"]);
    assert_eq!(
...
fn read_result() -> ReadResourceResult {
    serde_json::from_value(json!({
        "contents": [
            {"uri": "ui://w/a", "mimeType": "text/plain", "text": "not it"},
            {
                "uri": "ui://w/a",
