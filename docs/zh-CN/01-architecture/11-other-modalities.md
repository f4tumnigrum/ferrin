...
- 【事实】本地工具执行：`FunctionCallArgumentsDone` 事件对应的工具存在且可执行时，解析并校验参数后在会话内执行，输出以 `FunctionCallOutput` 提交；工具存在但无执行函数时不自动处理（应用调用 `add_tool_output`）；工具不存在为 `Err(Error::NoSuchTool)`；参数不合法为 `Err(Error::InvalidToolInput)`。同一响应内的多个工具调用全部提交输出且收到 `ResponseDone` 后，只发送一次 `ResponseCreate`。`health_check_response` 有返回时先回复再解析事件。
