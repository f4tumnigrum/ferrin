...
[Decision] Reference alignment (2026-09-17, ADR 0026): provider-option overlays use recursive object merging, array replacement and the reference reserved-key exclusions at every object boundary. A malformed registry model id produces `NoSuchModel`; a well-formed id with an unknown provider produces `NoSuchProvider`. `register_provider` replaces a provider for subsequent lookups without changing model handles already returned.

[Decision] Reference middleware metadata boundaries follow local AI SDK `6c6c221`: JSON extraction forwards unmatched text deltas unchanged; simulated text events omit the source part metadata while reasoning-start retains it; matched non-streaming reasoning extraction produces new text/reasoning parts without their source metadata. Result-level metadata remains unchanged. Empty tool descriptions are treated as absent when appending examples. JSON fence whitespace follows ECMAScript whitespace rules. Sources: reference `extract-json-middleware.ts`, `simulate-streaming-middleware.ts`, `extract-reasoning-middleware.ts`, `add-tool-input-examples-middleware.ts`; local middleware regressions.

[Decision] Registry files/skills lookups use a provider ID without a model separator and distinguish unknown providers from unsupported services. A custom provider's explicit files/skills interfaces take precedence over its fallback. Sources: reference `provider-registry.ts` and `custom-provider.ts`, local registry service regressions. This completes the interfaces described in sections 2.1 and 2.2.

[Decision] Default JSON response formats merge their schema objects recursively and retain omitted name/description fields, matching the reference `mergeObjects(settings, params)` behavior. Explicit schema arrays/scalars replace defaults. Source: local AI SDK `6c6c221`, `default-settings-middleware.ts` and `util/merge-objects.ts`; local default-settings regressions.
