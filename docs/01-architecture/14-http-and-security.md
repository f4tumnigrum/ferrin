...
[Decision] `WebSocketConfig` debug output redacts the complete URL and protocol list because Realtime credentials can appear in either location. Its connection values remain unchanged; debug formatting must not reveal tokens supplied to a provider's `websocket_config`.
