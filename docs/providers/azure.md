...
[Fact] Authentication is lazy: explicit `api_key` wins over `AZURE_API_KEY`; alternatively `token_provider` obtains an Entra token for each request that lacks an Authorization header. Supplying both explicit methods is invalid. `resource_name` or `AZURE_RESOURCE_NAME` constructs `https://<resource>.openai.azure.com/openai`; `base_url` overrides it. Source: `settings.rs`, `provider.rs`, `transport.rs`.
...
[Decision] Credentials are restricted to the configured origin and API path prefix. Cross-origin URLs, sibling paths, URL credentials, decoded path traversal, backslashes, control characters and ambiguous double encoding receive no Azure credential. Existing download URL validation remains in force. Within that boundary, configured and per-call headers override generated credential headers; an explicit Authorization header bypasses the Entra callback. Errors and debug output never expose credential values.

[Decision] Migration under [ADR 0026](../04-decisions/2026-09-17-0026-reference-sdk-parity.md): callers that require Entra refresh on every request must remove explicit Authorization headers. The earlier implementation replaced caller credentials unconditionally; header precedence now follows the reference Azure adapter.
