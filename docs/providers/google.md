...
[Decision] The seven existing tool factories use the reference object-input and object-output schemas. Empty tool schemas accept objects and strip undeclared fields; code execution retains only its declared string fields. Sources: local AI SDK `6c6c221`, `packages/google/src/tool`; `tests/suite/tools.rs` parser regressions. This is local validation coverage only.

[Decision] CamelCase keys; unknown keys are ignored, matching reference Zod object parsing, while invalid known fields return InvalidArgument. Schemas are in `src/options.rs`. Source: local AI SDK `6c6c221` option schemas.

[Decision] Modality option validation follows the local AI SDK `6c6c221` schemas: embedding task types use the documented enum, multimodal embedding entries contain nonempty lists of text/inline-data/file-data parts, speech speaker entries require their names and prebuilt voice names, and image grounding/configuration values must be objects. Invalid inputs fail before HTTP; required embedding response arrays are never synthesized from missing fields. Sources: `packages/google/src/google-{embedding,speech,image}-model-options.ts` in the local reference; local regressions verify conversion only.

[Decision] Speech responses accept omitted or null inline audio media types and data, as the reference speech schema does. The first nonempty audio payload is used; an absent media type yields the default 24 kHz sample rate and null `mimeType` metadata. Source: local AI SDK `google-speech-api.ts` and `google-speech-model.ts`; local fixture verification only.

[Decision] Batch status treats omitted pending/successful/failed counters as zero when the supplied total agrees, matching `convertGoogleRequestCounts`. Video download URLs use the resolved configured API-key header on the same origin, including a configured header override. Source: local AI SDK `google-batch.ts` and `google-video-model.ts`; credential isolation remains mandatory.

[Decision] Video request conversion omits zero `seed` and zero `duration`, matching the reference adapter's truthy checks. Use nonzero values when those fields must appear on the wire. Source: local AI SDK `google-video-model.ts` at `6c6c221`.
...
- [Fact] Images reject non-gemini IDs, masks, and n>1. Size warns to use `aspectRatio`; the advertised per-call image maximum defaults to ten and accepts `with_max_images_per_call` overrides, matching the reference even though Gemini rejects `n > 1`. Multi-image core calls must explicitly set `.max_images_per_call(1)` to split into supported single-image requests. Speech ignores `speed`/`language` with warnings, prefixes `instructions` unless multi-speaker (warn), and returns raw PCM with an explanatory warning when requested.
...
- [Decision] Files poll `state`; timeout in `PROCESSING` or `FAILED` returns `ApiCall`. `filename` is unsupported and emits a warning; use `displayName` for the Files API name. Polling options must be positive. Resource names are encoded as path data, including dot segments, following local AI SDK `google-files.ts`. Metadata retains `sizeBytes` as a string while normalized `byte_size` is numeric.
...
[Decision] Under ADR 0026, Live API function outputs retain every valid JSON value directly and replace invalid JSON with `{}`, matching reference `google-realtime-event-mapper.ts`. This replaces the earlier scalar/text wrapper. Normalized clear, response-create, response-cancel, truncate and audio-message events return `null`; the core skips sending them because Live API has no corresponding operation. Regressions: `tests/suite/realtime.rs::{function_outputs_preserve_json_and_replace_invalid_text,client_events_serialize_to_the_wire_format}` (2026-09-17).
...
[Decision] Interactions built-in result steps retain native JSON objects, arrays and scalar values when replayed; client `function_result` blocks serialize JSON to text. MCP result replay also retains the tool name. This preserves the distinct wire schemas rather than applying client-result serialization to provider results. Source: Interactions result schemas in local AI SDK `6c6c221` and `src/interactions/prompt.rs`; no live verification.

[Decision] Reference request normalization omits empty stop sequences and does not derive Interactions `thinking_level` from generic reasoning effort; use `thinkingLevel`. Deprecated `imageConfig` supplies an image response format only when no explicit image entry exists. Linked-history compaction removes results whose tool-call message was compacted, while unrelated new results remain. Adjacent user text blocks merge with a blank line. Sources: local AI SDK Interactions request and prompt converters (`6c6c221`).
...
[Fact] Functions and Google search/code execution/URL context/file search/maps/computer use/retrieval/MCP tools have Interactions wire conversion and built-in aliases restore application names. Tool conversion selects each documented field, omits null values, and applies the reference computer-use and retrieval defaults. Remote `mcp_server` URLs are forwarded to Google for execution; no local connection is opened. Unknown provider tools warn and are omitted. Unary and streaming results include text, reasoning, files, calls/results, URL/document/maps citations, token counts and service tiers; streaming deduplicates sources. Sources: local AI SDK `6c6c221`, `prepare-google-interactions-tools.ts`; `src/interactions/tools.rs`, `output.rs`, `sources.rs` and fixture regressions. No live provider verification.
...
[Decision] Migration under ADR 0026 restores the reference image maximum of ten. Existing multi-image callers relying on automatic single-image splitting must set `.max_images_per_call(1)`; otherwise the adapter rejects the resulting `n > 1` request before HTTP. Source: local AI SDK `6c6c221`, `packages/google/src/google-image-model.ts`.

[Decision] Interactions option normalization preserves explicit provider response formats for agents while dropping only generic JSON structured output. Modality-specific formats, agent configuration, and remote environment objects serialize only reference-defined fields; null optional fields and empty environment source lists are omitted. Dynamic agent configuration emits only its type. Invalid format discriminants and malformed environments fail before HTTP. Source: local AI SDK `6c6c221`, Interactions language model and option schema.
