...
[Decision] Function and structured-output schemas are forwarded unchanged, matching local AI SDK `6c6c221`; function `strict` is sent only when supplied and `strictJsonSchema` controls the structured-output flag (default `true`). Disabled `supports_structured_outputs` keeps the `json_object` fallback. This replaces automatic strict transforms recorded on 2026-09-15 under [ADR 0026](../04-decisions/2026-09-17-0026-reference-sdk-parity.md); the explicit transformation API in [ADR 0019](../04-decisions/2026-09-15-0019-fallible-schema-transforms.md) remains available. Source: `strict_schema` request regressions; no live API verification.
