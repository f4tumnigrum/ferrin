...
## Provider additions in 0.2.0 (2026-09-18)
...
[Decision] Application callbacks and results expose contexts; Telemetry excludes both contexts by default. Opt in separately with `TelemetryOptions::include_runtime_context` and `include_tools_context`, using `..Default::default()` for remaining defaults. New provider features are optional and are not enabled automatically. These breaking changes are included in the coordinated 0.2.0 release; see the release record for publication verification.

### Reference alignment (ADR 0026)

[Fact] Every `Telemetry` lifecycle callback now returns `BoxFuture<'a, ()>` and is awaited. Migrate implementations from a synchronous body to `Box::pin(async move { ... })`; integration callbacks settle concurrently with panic isolation. `ToolExecutionContext` adds `record_outputs`, which wrappers must respect before recording result content. Sources: `ferrin-core/src/telemetry/{mod,dispatcher}.rs`, `ferrin-otel/src/telemetry.rs`.

[Fact] `embed`, `embed_many` and `rerank` accept `.runtime_context(...)`, `.on_start(...)` and `.on_end(...)`. These hooks run once per logical operation outside chunk retries; embedding events distinguish single and multiple inputs/results, and successful empty reranking also emits both events. Integrations use `Telemetry::on_embed_operation_start/end` and `on_rerank_operation_start/end`, which receive filtered copies concurrently with application hooks. The existing telemetry attempt callbacks remain separate. Sources: `ferrin-core/src/{modality_hooks,embed,rerank}.rs` and `telemetry/dispatcher/modalities.rs`.

[Fact] Embedding components and `cosine_similarity` now use `f64`, matching the reference number precision. Convert existing `Vec<f32>` values explicitly. `Instructions` is a non-exhaustive enum with `System(SystemMessage)` and `Messages(Vec<SystemMessage>)`; prefer `.into()` for text or system-message vectors. Empty instruction arrays remain empty, and each message retains its provider options. Sources: `ferrin-spec/src/embedding_model.rs`, `ferrin-core/src/{embed,prompt/standardize}.rs`.

[Fact] `PrepareStepContext::model` now exposes the configured model instance; use its `provider()` / `model_id()` methods when identity is needed. Initial instructions and sandbox are available during preparation; a sandbox override applies to one step. Agent `.call_options_schema(schema)` validates and normalizes serialized options before `prepare_call`; serialization is required only when this method is used. `PreparedCall` also supports approval settings, tool callers, repair/refinement, step preparation and downloads. Sources: `ferrin-core/src/agent/{options,prepare_call,tool_loop_agent}.rs`, `generate_text/prepare_step.rs`.

[Decision] Lifecycle hooks settle concurrently and isolate synchronous and asynchronous panics. Do not use callback registration order to coordinate side effects. Explicit call timeout settings override prepared agent timeout settings. See [Agent](../01-architecture/09-agent.md) for the revised ordering contract.

[Fact] `StreamTextResult::final_result()` and the completion returned by `split()` drive the pipeline themselves. `full_stream`, `text_view`, `partial_output_view` and `element_view` create independent views at the current cursor; lagging views retain buffered events until consumed or dropped. `into_shared_completion()` returns cloneable waiters sharing the final allocation without requiring the output to be `Clone`. Dropping the last owner cancels pending work. Sources: `ferrin-core/src/stream_text/result.rs` and `result/tee.rs`.

[Fact] `GenerateTextResult::warnings()` now returns references from all steps; usage also aggregates all steps. `RerankResult::original_documents` retains the supplied documents. `RequestBody::to_bytes()` and multipart `encode()` now return `Result`; `into_stream()` preserves one-shot streaming ownership. Sources: `ferrin-core/src/{generate_text/result,rerank}.rs`, `ferrin-provider-util/src/http/{request_body,multipart}.rs`.

[Decision] OpenAI and compatible adapters preserve supplied JSON schemas instead of applying `OpenAiStrict` automatically. Apply the explicit schema transform before passing a schema when that transformation is needed. Provider-defined tool factories use reference input schemas, including object defaults and stripping of undeclared fields; `Schema::from_json_schema` remains validation-only. See [OpenAI](../providers/openai.md) and [OpenAI-compatible](../providers/openai-compatible.md).

[Fact] Policy default input is now `{tool: {name}, args, messages, runtimeContext}`. Shadow observation permits execution and sends the original decision to its observer; use `.on_decision(...)` for asynchronous observers or `.on_decision_sync(...)` for synchronous observers and `flush_decisions().await` when observation must finish before shutdown. Sources: `ferrin-policy/src/{approval,decision,shadow}.rs`.

[Fact] MCP multi-round tool input and cancellation notifications now default off; set `McpClientConfig::max_input_rounds` and `send_cancel_notifications` explicitly to retain those extensions. `initialization_timeout` covers transport startup, discovery and initialization. OAuth callbacks accept `AuthOptions::callback_state` / `callback_issuer`; providers can persist state and authorization-server information through the new storage hooks. Token/client expiration and issuance values use `f64`, authorization resources are optional, and stored credentials include optional `OAuthAuthorizationServerInformation`. Existing unbound tokens are invalidated before use. Sources: `ferrin-mcp/src/client/mod.rs`, `oauth/{auth,flow,provider,types}.rs`; see [MCP](../01-architecture/15-mcp.md) for remaining differences.
